...
public class RSAUtil {
    ...
    private static byte[] buildPKCS8Key(String privateKey) throws IOException {
        if (privateKey.contains("-----BEGIN PRIVATE KEY-----")) {
            return Base64Utils.decode(privateKey.replaceAll("-----\\w+ PRIVATE KEY-----", ""));
        } else if (privateKey.contains("-----BEGIN RSA PRIVATE KEY-----")) {
            final byte[] innerKey = Base64Utils.decode(privateKey.replaceAll("-----\\w+ RSA PRIVATE KEY-----", ""));
            ...
            return Base64Utils.decode(privateKey);
        }
    }
