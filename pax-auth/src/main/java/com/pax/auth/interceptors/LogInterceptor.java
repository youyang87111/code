package com.pax.auth.interceptors;
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response,
		...
		if (handler instanceof HandlerMethod) {
			...
			if (!isIgnore(controllerName + "." + methodName, ignores)) {
				...
				//设置操作者
				String loginuser = AuthUtils.getUserLoginname();
				...
				if (StringUtils.isNotBlank(errorMsg)) {
					...
					log.setDescription(TranslationUtils.getInstance(com.pax.common.util.WebUtils.getSession() == null ? null : (Locale) com.pax.common.util.WebUtils.getSession().getAttribute("locale")).__("操作成功"));
					log.setFlag("0");
				}
