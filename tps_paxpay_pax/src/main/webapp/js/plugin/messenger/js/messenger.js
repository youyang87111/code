...
(function() {
    ...
    _Message = (function(_super) {
        ...
        _Message.prototype.show = function() {
            ...
            this.$el.parent().css({left:($(window).width()-this.$el.parent().width())/2});
        ...
        _Message.prototype.actionsToEvents = function() {
            ...
            for (name in _ref2) {
                act = _ref2[name];
                _results.push(this.events["click [data-action=\"" + name + "\"]"] = (function(act) {
                    return function(e) {
                        e.preventDefault();
                        e.stopPropagation();
        ...
        _Message.prototype.template = function(opts) {
            var $action, $actions, $cancel, $link, $message, $text, action, _i, _len, _ref2,
                _this = this;
            $title = jQuery.i18n.prop('warning_tip');//警示
            switch(opts.type)
            {
                case "error":
                case "alert":
                   $title = jQuery.i18n.prop('warning_tip');//警示
                    break;
                case "info":
                   $title = jQuery.i18n.prop('error_data_comfirm_1');//确定
                    break;
                case "success":
                   $title = jQuery.i18n.prop('success_tip');//成功
                    break;
                default:
                   $title = "确定";//警示
                    break;
            ...
            $message.append("<div class='messenger-message-header'><h4>"+$title+"</h4></div>");
            ...
            if (opts.escapeText) {
                $text = $('<div class="messenger-message-inner"><i class="fa"></i></div>').append("<span>").text(opts.message);
            } else {
                $text = $("<div class=\"messenger-message-inner\"><table><tr><td><i class='fa'></i></td><td style='padding-left:20px;'><span>" + opts.message + "</span></td></tr></table></div>");
            }
            $message.append($text);
            if (opts.actions.length) {
                $actions = $('<div class="messenger-actions btn-group">');
            }
            _ref2 = opts.actions;
            for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
                action = _ref2[_i];
                $action = $('<button class="btn btn-primary">');
    ...
    window.Messenger._call = function(opts) {
        ...
        defaultOpts = {
            extraClasses: 'messenger-fixed  messenger-on-center messenger-on-right',
            theme: 'future',
            maxMessages: 9,
        ...
        if (opts.instance == null) {
            ...
            if (!inst) {
                $el = $('<ul>');
                $sh = $('<div class="modal fade modal-primary in" style="z-index:9999;">');
                $sh.append($el);
                $parent.prepend($sh);
