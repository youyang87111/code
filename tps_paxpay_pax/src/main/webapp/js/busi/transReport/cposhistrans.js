    ...
    // PAX_FUNCTION.getCommons("#innid_s",contextPath+"/common/getInnid?classGroup=01",{},"id","name");
     ...
     $("#export").click(function(){//导出
        ...
        if(transdateb_ > transdatee_){
         ...
         PAX_FUNCTION.ajaxGet(contextPath+"/cposhistrans/exportCount",reqData,
             function(data){
                 if(data.msg>65535){
                     _load.stop();
                 ...
                 window.location = contextPath+"/cposhistrans/export?search_bid="+$("#bid_s").val()+"&search_mid="+$("#mid_s").val()+"&search_tid="+$("#tid_s").val()+"&search_rbid="+$("#rbid_s").val()+"&search_rmid="+$("#rmid_s").val()+"&search_rtid="+$("#rtid_s").val()+"&search_innid="+$("#innid_s").val()+"&search_sysid="+$("#sysid_s").val()+"&search_orgId="+$("#cposhistransOrgId").val()+"&search_transdateb="+transdateb_s+"&search_transdatee="+transdatee_s+"&search_amountb="+$("#amountb_s").val()+"&search_amounte="+$("#amounte_s").val();
