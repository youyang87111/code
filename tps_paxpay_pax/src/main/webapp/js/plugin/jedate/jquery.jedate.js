...
;(function(root, factory) {
	...
	jedfn.setHtml = function(opts){
		...
		var dateymList = !isYYMM ? '<div class="jedatetopym" style="display: none;">' + '<ul class="ymdropul"></ul><p><span class="jedateymchle">&lt;&lt;</span><span class="jedateymchri">&gt;&gt;</span><span class="jedateymchok">'+I("closeText")+'</span></p>' + "</div>" :(dateFormat == "YYYY" ? '<ul class="jedayy"></ul>' :　'<ul class="jedaym"></ul>');
		var dateriList = '<ol class="jedaol"></ol><ul class="jedaul"></ul>';
		var bothmsStr = !isYYMM ? '<div class="botflex jedatehmsshde"><ul class="jedatehms"><li><input type="text" /></li><i>:</i><li><input type="text" /></li><i>:</i><li><input type="text" /></li></ul></div>' + '<div class="botflex jedatebtn"><span class="jedateok">'+I("confirmText")+'</span><span class="jedatetodaymonth">今天</span><span class="jedateclear">'+I("clearText")+'</span></div>' :(dateFormat == "YYYY" ? '<div class="botflex jedatebtn"><span class="jedateok" style="width:47.8%">'+I("confirmText")+'</span><span class="jedateclear" style="width:47.8%">'+I('clearText')+'</span></div>' : '<div class="botflex jedatebtn"><span class="jedateok">'+I("confirmText")+'</span><span class="jedatetodaymonth">'+I("currentMon")+'</span><span class="jedateclear">'+I("clearText")+'</span></div>');
		...
		if(/\hh-mm/.test(dateFormat)){
			...
			if(ishhmm){
				isTimehms(true);
				boxCell.find(".jedateyear").text(jet.currDate.getFullYear() + I('yearText'));
				boxCell.find(".jedatemonth").text(jet.digit(jet.currDate.getMonth() + 1) + I('monText'));
			}else{
		...
		if(isYYMM){
			...
			if(dateFormat == "YYYY"){
				monthCls.attr("data-onyy",tmsArr[0]).text(tmsArr[0] + I('yearText'));
				boxCell.find(".jedayy").html(that.onlyYear(tmsArr[0]));
			}else{
				monthCls.attr("data-onym",tmsArr[0]+"-"+jet.digit(tmsArr[1])).text(tmsArr[0] + I('yearText') + parseInt(tmsArr[1]) +I('monText'));
	...
	jedfn.createDaysHtml = function(ys, ms, opts){
		...
		boxCell.find(".jedateyear").attr("year", year).text(year + I('yearText'));
		boxCell.find(".jedatemonth").attr("month", month).text(month + I('monText'));
		//设置时间标注
	...
	jedfn.onlyYMStr = function(y, m) {
		...
		$.each(jet.montharr, function(i, val) {
			...
			if (thisDate < minTime || thisDate > maxTime) {
				onlyYM += "<li class='disabled' ym='" + y + "-" + jet.digit(val) + "'>" + y + I('yearText') + jet.digit(val) + I('monText')+"</li>";
			} else {
				onlyYM += "<li " + (m == val ? 'class="action"' :"") + ' ym="' + y + "-" + jet.digit(val) + '">' + y + I('yearText') + jet.digit(val) + I('monText')+"</li>";
			}
		});
		return onlyYM;
	};
	...
	jedfn.onlyYear = function(YY) {
		...
		$.each(jet.yearArr, function(i) {
			...
			if (yyi < minY || yyi > maxY) {
				onlyStr += "<li class='disabled' yy='" + yyi + "'>" + yyi + I('yearText')+"</li>";
			} else {
				onlyStr += "<li "+(getyear == yyi ? 'class="action"' : "")+" yy='" + yyi + "'>" + yyi + I('yearText')+"</li>";
			}
		});
		return onlyStr;
	};
	...
	jedfn.chooseDays = function(opts) {
		...
		if(opts.festival) {
			...
			boxCell.find(".jedaul li").on("mouseover", function () {
				...
				var tiphtml = '<p>' + lunar.solarYear + I('yearText') + lunar.solarMonth + I('monText') + lunar.solarDate + '日 ' + lunar.inWeekDays + '</p><p class="red">农历：' + lunar.shengxiao + '年 ' + lunar.lnongMonth +I('monText') + lunar.lnongDate + '</p><p>' + lunar.ganzhiYear + '年 ' + lunar.ganzhiMonth + I('monText') + lunar.ganzhiDate + '日</p>';
	...
	jedfn.chooseYM = function(opts) {
		...
		function eachYearMonth(YY, ymlen) {
			...
			if (ymlen == 12) {
				$.each(jet.montharr, function(i, val) {
					...
					ymStr += "<li " + mcls + ' mm="' + val + '">' + val + I('monText')+"</li>";
				});
		...
		function clickLiYears(year) {
			boxCell.find(".ymdropul li").on("click", function(ev) {
				...
				ev.stopPropagation();
				year.attr("year", Years).html(Years + I('yearText'));
		...
		!ishhmmss && jedatemm.on("click", function() {
			...
			boxCell.find(".ymdropul li").on("click", function(ev) {
				...
				jedatemonth.attr("month", Months).html(Months + I('monText'));
				jet.isShow(jetopym,false);
				that.createDaysHtml(Years, Months, opts);
			});
		});
...
;(function(root, factory) {
	...
	function flunar(Y) {
		...
		if (this.lunarDate == 1) {
			this.showInLunar = this.lnongMonth + I('monText');
		}
...
$("#rebate_prof").jeDate({
	...
	festival:true,
	ishmsVal:false,
	minDate: '2016-10-1 23:59:59',
	maxDate: $.nowDate(0),
	format:"YYYY-MM",
	zIndex:3000,
})
function _T2(textKey) {
